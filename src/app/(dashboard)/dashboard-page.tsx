      ...
      <Card>
        ...
        <CardContent>
          {contasAPagar.length === 0 ? (
            ...
            <div className="grid grid-flow-col auto-cols-[82%] gap-2 overflow-x-scroll overscroll-x-contain pb-2 snap-x snap-mandatory touch-pan-x [-webkit-overflow-scrolling:touch]">
              {contasAPagar.slice(0, 8).map((conta) => (
                <div
                  key={conta.id}
                  className={`min-w-0 rounded-md border p-2 snap-start ${conta.status === "ATRASADA" ? "border-red-300 bg-red-50" : "border-yellow-200 bg-yellow-50"}`}
                >
